...
function PA2_xm93woodland_Zones()
    local dirs = getLotDirectories()
    for i=dirs:size(),1,-1 do
        local map = dirs:get(i-1)
        if map == "Muldraugh, KY" then
            getWorld():registerVehiclesZone("xm93woodland", "ParkingStall", 14541, 4025, 0, 3, 3, { Direction = "E" })
        end
    end
end


Events.OnLoadMapZones.Add(PA2_xm93woodland_Zones)
